...
class CommunityNetworkTopologyView (TemplateView):
    template_name = 'project/community_network_topology/index.html'
...
class JSONView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            if self.request.user.is_superuser:
                firewalls = api.fwaas.firewall_list(request)
            else:
                firewalls = api.fwaas.firewall_list_for_tenant(
                    request, request.user.tenant_id)
        except:
            firewalls = []

        # api.rest.utils.ensure_firewall_rules(request, firewalls)

        data['firewalls'] = [{
            'id': firewall.id,
            'name': firewall.name,
            'status': firewall.status,
            'routers': firewall.router_ids,
            'rules': firewall['policy']['firewall_rules']
