...
(function() {
  ...
  function(loadbalancerAPI, smodal, toastService) {
    ...
    var context = {
      title: gettext('Enable listeners'),
      message: gettext('The amount of listeners these will be enabled is : %s'),
      tips: gettext('Please confirm your selection. This action cannot be undone.'),
      submit: gettext('Enable listeners'),
      success: gettext('enabled listeners: %s.'),
      error: gettext('Unable to enable listeners %s: %s.')
    };
    ...
    function action(scope) {
      ...
      self.singleEnable = function(listener) {
        self.confirmEnable([listener.id], [listener.name]);
      };
      ...
      self.batchEnable = function(table) {
        ...
        self.confirmEnable(listeners, names);
      };

      // brings up the confirmation dialog
      self.confirmEnable = function(listeners, names) {
        var options = {
          title: context.title,
          tips: context.tips,
      ...
      self.submit = function(listeners) {
        for(var i = 0; i < listeners.length; i++){
          self.enableListener(listeners[i]);
        }
      ...
      self.enableListener = function(listener) {
