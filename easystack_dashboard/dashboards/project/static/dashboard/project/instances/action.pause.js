...
(function() {
  ...
  /**
   * @ngDoc pauseInstanceAction
   * @ngService
   *
   * @Description
   * Brings up the pause instances confirmation modal dialog.

   * On submit, pause selected instances.
   * On cancel, do nothing.
   */
  .factory('pauseInstanceAction', ['horizon.openstack-service-api.nova',
                                   'horizon.framework.widgets.modal.service',
                                   'horizon.framework.widgets.toast.service',
  function(novaAPI, smodal, toastService) {

    var context = {
      title: gettext('Confirm Pause Instance'),
      message: gettext('You have selected %s.'),
      tips: gettext('Please confirm your selection.'),
      submit: gettext('Pause Instances'),
      success: gettext('Pause Instances: %s.'),
      error: gettext('Pause Instances: %s.')
    };
    ...
    function action(scope) {
      ...
      self.singlePause = function(instance) {
        self.confirmPause([instance.id], [instance.name], [instance]);
      };

      // delete selected volume objects
      // action requires the volume to select rows
      self.batchPause = function() {
        var instances = [];
        var names = [];
        ...
        self.confirmPause(names, instances);
      };

      // brings up the confirmation dialog
      self.confirmPause = function(names, instances) {
      ...
      self.submit = function(instances) {
        scope.doAction(context, instances, novaAPI.pauseServer);
      };
