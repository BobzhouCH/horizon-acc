...
class Settings(generic.View):
    ...
    def get(self, request):
        return dict((k, getattr(settings, k, None)) for k in settings_allowed)

@urls.register
class Setting(generic.View):
    """API for retrieving settings.
    ...
    Examples of settings: OPENSTACK_HYPERVISOR_FEATURES
    """
    url_regex = r'setting/(?P<id>[0-9A-Za-z_]+)$'

    @rest_utils.ajax()
    def get(self, request, id):
        return getattr(settings, id, None)
