...
class Resource(base.APIResourceWrapper):
    ...
    def resource_name(self):
        return self._apiresource.resource_name

    @property
    def type(self):
        if self.metadata.has_key('volume_type'):
            self._type = 'volume'
        elif self.metadata.has_key('mac'):
            self._type = 'nic'
        elif self.metadata.has_key('disk_name'):
            self._type = 'disk'
        elif self.metadata.has_key('instance_type'):
            self._type = 'instance'
        return self._type
...
class Sample(base.APIResourceWrapper):
    ...
    _attrs = ['counter_name', 'resource_name', 'user_id', 'resource_id', 'timestamp',
...
class Event(base.APIResourceWrapper):
    ...
    _attrs = ['event_type', 'generated', 'message_id', 'raw', 'traits',
              'resource_name', 'resource_id', 'image_type']
...
class Alarm(base.APIResourceWrapper):
    ...
    _attrs = ['alarm_actions', 'alarm_id', 'comparison_operator',
              ...
              'statistic', 'threshold', 'timestamp', 'user_id', 'severity']
    ...
    def __init__(self, apiresource, ceilometer_usage=None):
        ...
        if hasattr(apiresource, 'threshold_rule'):
	     field = apiresource.threshold_rule.get('query')
             if len(field) > 0:
                 field = field[0].get('field')
                 if field == 'resource_id':
                     self._resourceid = apiresource.threshold_rule.get(
                         'query')[0].get('value')
    ...
    def meter_name(self):
        meter = ""
        rule = getattr(self._apiresource, 'threshold_rule', None)
        if rule:
            meter = rule['meter_name']
        return meter

#NFVI alerts --START
class Alert(base.APIResourceWrapper):

    """ Represents one Ceilometer alert.
    """
    ...
    def __init__(self, apiresource, ceilometer_usage=None):
        super(Alert, self).__init__(apiresource)
...
def query_alarms(request, filter=None, order_by=None, limit=None,
                 ceilometer_usage=None):
    """Query alarms directly from the db based on the filter parameter."""
    alarms = ceilometerclient(request, 'alarming').\
        query_alarms.query(filter=filter, orderby=order_by, limit=limit)
    return [Alarm(alarm, ceilometer_usage) for alarm in alarms]


def alarm_list(request, query=None, ceilometer_usage=None):
    """List of alarms."""
    #LOG.debug("nfvpoc-alarm-list:%s" % request) 
    alarms = ceilometerclient(request, 'alarming').\
        alarms.list(q=query)
    return [Alarm(alarm, ceilometer_usage) for alarm in alarms]
...
def alarm_get(request, alarm_id, ceilometer_usage=None):
    """Get an alarm."""
    alarm = ceilometerclient(request, 'alarming').alarms.get(alarm_id)
    resourceid = Alarm(alarm, ceilometer_usage).resourceid
    return [alarm, resourceid]


def alarm_history(request, alarm_id):
    """Get an alarm history."""
    history = ceilometerclient(request, 'alarming').alarms.get_history(alarm_id)
    return [h for h in history]


@base.create_log_decorator(
    optype='Create', subject='Alarm', detail=None)
def alarm_create(request, ceilometer_usage=None, **kwargs):
    """Create an alarm."""
    alarm = ceilometerclient(request, 'alarming').\
...
def alarm_delete(request, alarm_id):
    """Delete an alarm."""
    ceilometerclient(request, 'alarming').\
        alarms.delete(alarm_id)


#NFVI alerts --STRAT
def query_alerts(request, filter=None, order_by=None, limit=None,
                 ceilometer_usage=None):
    """Query alerts directly from the db based on the filter parameter."""
    alerts = ceilometerclient(request).\
        query_alarms.query(filter=filter, orderby=order_by, limit=limit)
    return [Alert(alert, ceilometer_usage) for alert in alerts]


def alert_list(request, query=None, ceilometer_usage=None):
    """List of alerts."""
    alerts = ceilometerclient(request, 'alarming').\
        alarms.list(q=query)
    return [Alert(alert, ceilometer_usage) for alert in alerts]


def alert_get(request, alert_id, ceilometer_usage=None):
    """Get an alert."""
    alert = ceilometerclient(request, 'alarming').alarms.get(alert_id)
    resourceid = Alert(alert, ceilometer_usage).resourceid
    return [alert, resourceid]


def alert_history(request, alert_id):
    """Get an alert history."""
    history = ceilometerclient(request).alarms.get_history(alert_id)
    return [h for h in history]
#NFVI alerts --END
...
@memoized
def ceilometerclient(request, endpoint_type='metering'):
    """ Initialization of Ceilometer client.
    """

    endpoint = base.url_for(request, endpoint_type)
...
def resource_list(request, query=None, ceilometer_usage_object=None):
    ...
    list = []
    for r in resources:
        if ('volume_type' in r.metadata) and (r.metadata.get('status') != 'error'):
            list.append(Resource(r, ceilometer_usage_object))
        elif ('instance_type' in r.metadata) and ('terminated_at' not in r.metadata):
            list.append(Resource(r, ceilometer_usage_object))
    return list


def resource_get(request, resourceid):
    """Get resource by id."""
    try:
        resource = ceilometerclient(request).\
            resources.get(resource_id=resourceid)
    except Exception:
        resource = None
    if resource is None:
        return resource
    if ('volume_type' in resource.metadata) and (resource.metadata.get('status') != 'error'):
        return Resource(resource)
    elif ('instance_type' in resource.metadata) and ('terminated_at' not in resource.metadata):
        return Resource(resource)
    return resource
...
def events_list(request, query=None, limit=None):
    events = ceilometerclient(request).query_events.query(
        filter=query,
        orderby='[{"timestamp": "desc"}]',
        limit=limit)
    return [Event(e) for e in events]

def realtime_show(request, meter_name, query=None):
    """List the samples for this meters."""
    #LOG.debug("nfvpoc-realtime_show-m:%s,q:%s" % (meter_name, query)) 
    samples = ceilometerclient(request).realtime.show(meter_name=meter_name,
                                                      q=query)
    return [Sample(s) for s in samples]
...
def statistic_list(request, meter_name, query=None, period=None, groupby=None):
    """List of statistics."""
    #LOG.debug("nfvpoc-statistic_list:q:%s" % query) 
    statistics = ceilometerclient(request).\
        statistics.list(
            meter_name=meter_name, q=query, period=period, groupby=groupby)
...
class CeilometerUsage(object):
    ...
    def resource_aggregates_with_statistics(
            self, queries=None,
...
class Meters(object):
    ...
    def __init__(self, request=None, ceilometer_meter_list=None):
        ...
        all_services_meters = (
            self._nova_meters_info,
            self._neutron_meters_info, self._glance_meters_info,
            self._cinder_meters_info, self._swift_meters_info,
            self._kwapi_meters_info, self._ipmi_meters_info)
        self._all_meters_info = {}
        for service_meters in all_services_meters:
            self._all_meters_info.update(
                dict([(meter_name, meter_info)
                      for meter_name, meter_info in service_meters.items()]))
    ...
    def list_nova(self, except_meters=None):
        ...
        return self._list(
            only_meters=self._nova_meters_info.keys(),
            except_meters=except_meters)
    ...
    def list_neutron(self, except_meters=None):
        ...
        return self._list(
            only_meters=self._neutron_meters_info.keys(),
            except_meters=except_meters)
    ...
    def list_glance(self, except_meters=None):
        ...
        return self._list(
            only_meters=self._glance_meters_info.keys(),
            except_meters=except_meters)
    ...
    def list_cinder(self, except_meters=None):
        ...
        return self._list(
            only_meters=self._cinder_meters_info.keys(),
            except_meters=except_meters)
    ...
    def list_swift(self, except_meters=None):
        ...
        return self._list(
            only_meters=self._swift_meters_info.keys(),
            except_meters=except_meters)
    ...
    def list_kwapi(self, except_meters=None):
        ...
        return self._list(
            only_meters=self._kwapi_meters_info.keys(),
            except_meters=except_meters)
