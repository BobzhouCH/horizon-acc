...
VERSIONS = base.APIVersionManager("image", preferred_version=1)
...
def glanceclient(request, version=None):
    ...
    # TODO (braveliuchina): Temporarily keep both till we update the API calls
...
def _normalize_list_input(filters, **kwargs):
    ...
    if VERSIONS.active < 2:
        ...
        v1_keywords = ['page_size', 'limit', 'sort_dir', 'sort_key', 'marker',
                       'is_public', 'return_req_id', 'paginate', 'image_type']
...
@base.create_log_decorator(optype='Create', subject='Image', detail=None)
...
def image_update_properties(request, image_id, remove_props=None, **kwargs):
    ...
    '''Bug 105835: unable to update images' metadata. Need to use api version
     2 to call the update method.'''
...
def image_delete_properties(request, image_id, keys):
    ...
    return glanceclient(request).images.update(image_id, keys)
