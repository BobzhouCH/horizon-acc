...
from easystack_dashboard.api import network_base
...
class Server(base.APIResourceWrapper):
    ...
    _attrs = ['addresses', 'attrs', 'id', 'image', 'links',
              ...
              'OS-DCF:diskConfig', 'OS-SRV-USG:launched_at', 'security_groups',
              'os-extended-volumes:volumes_attached', 'host-compute-service-status',
              'root_device_name', 'root_gb']
    ...
    def image_name(self):
        ...
        from easystack_dashboard.api import glance  # noqa
    ...
    @property
    def launched(self):
        return getattr(self, 'OS-SRV-USG:launched_at', "")

    @property
    def volumes(self):
        return getattr(self, 'os-extended-volumes:volumes_attached', "")
...
class SecurityGroupManager(network_base.SecurityGroupManager):
    ...
    def delete(self, security_group_id):
        try:
            ...
            # web page may request the same sgr_id repeatedly, catch the repeat.
            pass
    ...
    def update_instance_security_group(self, instance_id,
        ...
        try:
            ...
            raise Exception(_("Couldn't get current security group list for instance %s.")
                            % instance_id)
        ...
        try:
            ...
            LOG.error(_("Failed to modify %(num_groups_to_modify)d instance security groups: %(err)s") %
...
def novaclient(request):
    ...
    c = nova_client.Client('2.0', request.user.username,
...
@base.create_log_decorator(
    optype='Create', subject='Flavor', detail=None)
def flavor_create(request, name, memory, vcpu, disk, flavorid='auto',
                  ephemeral=0, swap=0, metadata=None, is_public=True, rxtx_factor=1):
...
def pssr_get_ports(request,physerver):
    results = novaclient(request).pssr.pssr_get_ports(physerver)
    LOG.info("pssr_get_ports,results = %s",results._info)
    return results._info["results"]

def pssr_get_intfs(request,physerver):
    results = novaclient(request).pssr.pssr_get_intfs(physerver)
    LOG.info("pssr_get_intfs,results = %s",results._info)
    return results._info["results"]

def pssr_conf(request,request2):
    results = novaclient(request).pssr.pssr_conf(request2)
    LOG.info("pssr_conf,results = %s",results._info)
    return results._info["results"]

def pssr_get_port_conf(request,physerver,dev):
    results = novaclient(request).pssr.pssr_get_port_conf(physerver, dev)
    LOG.info("pssr_get_port_conf,results = %s",results._info)
    return results._info["results"]

def pssr_get_intf_conf(request,physerver,dev):
    results = novaclient(request).pssr.pssr_get_intf_conf(physerver, dev)
    LOG.info("pssr_get_intf_conf,results = %s",results._info)
    return results._info["results"]

def pssr_get_pfvf_num_configured(request,physnet):
    results = novaclient(request).pssr.pssr_get_pfvf_num_configured(physnet)
    LOG.info("pssr_get_pfvf_num_configured,results = %s",results._info)
    return results._info["results"]
...
def flavor_extra_delete(request, flavor_id, keys):
    ...
    flavor = novaclient(request).flavors.get(flavor_id)
    LOG.info("lhmflavor_extra_delete = %s", keys)
...
@base.create_log_decorator(
    optype='Create', subject='Instance Snapshot', detail=None)
@billing.create_product(ptype='image', uuid_name=None)
def snapshot_create(request, instance_id, name, metadata):
    return novaclient(request).servers.create_image(
        instance_id, name, metadata=metadata)


@base.create_log_decorator(
    optype='Create', subject='Keypair', detail=None)
...
@base.create_log_decorator(optype='Create', subject='Instance', detail=None)
@billing.create_product('instance', uuid_name='id', billing_by='flavor')
def server_create(request, name, image, flavor, key_name, user_data,
                  security_groups, block_device_mapping=None,
                  block_device_mapping_v2=None, nics=None,
                  availability_zone=None, instance_count=1, admin_pass=None,
                  disk_config=None, config_drive=None,
    ...
    return Server(novaclient(request).servers.create(
        ...
        scheduler_hints=scheduler_hints, meta=meta), request)


def ironic_server_create(request, name, image, flavor, key_name, user_data,
                  security_groups, block_device_mapping=None,
                  block_device_mapping_v2=None, nics=None,
                  availability_zone=None, instance_count=1, admin_pass=None,
...
@base.create_log_decorator(optype='Delete', subject='Instance', detail={'level':'high_level'})
def server_delete(request, instance):
    novaclient(request).servers.delete(instance)


def server_get(request, instance_id):
    nova = novaclient(request).servers.get(instance_id)
...
def server_list(request, search_opts=None, all_tenants=False):
    ...
    # call cache version of server list
    search_opts['redirect'] = 'detail_x'
...
@base.create_log_decorator(optype='Pause', subject='Instance', detail=None)
def server_pause(request, instance_id):
    novaclient(request).servers.pause(instance_id)


@base.create_log_decorator(optype='Unpause', subject='Instance', detail=None)
def server_unpause(request, instance_id):
    novaclient(request).servers.unpause(instance_id)


@base.create_log_decorator(optype='Suspend', subject='Instance', detail=None)
def server_suspend(request, instance_id):
    novaclient(request).servers.suspend(instance_id)


@base.create_log_decorator(optype='Resume', subject='Instance', detail=None)
def server_resume(request, instance_id):
    novaclient(request).servers.resume(instance_id)


@base.create_log_decorator(optype='Reboot', subject='Instance', detail=None)
...
@base.create_log_decorator(optype='Rebuild', subject='Instance', detail=None)
...
@base.create_log_decorator(optype='Update', subject='Instance', detail=None)
def server_update(request, instance_id, name):
    return novaclient(request).servers.update(instance_id, name=name)


@base.create_log_decorator(optype='Migrate', subject='Instance', detail=None)
def server_migrate(request, instance_id):
    novaclient(request).servers.migrate(instance_id)


@base.create_log_decorator(optype='Live Migrate', subject='Instance', detail=None)
...
@base.create_log_decorator(optype='Set Actual Total Memory', subject='Instance', detail=None)
def server_set_actual_total_memory(request, instance_id, size, granularity):
    novaclient(request).servers.set_actual_total_memory(instance_id, size, granularity)


@base.create_log_decorator(optype='Get Actual Total Memory', subject='Instance', detail=None)
def server_get_actual_total_memory(request, instance_id):
    return novaclient(request).servers.get_actual_total_memory(instance_id)


@base.create_log_decorator(optype='Hot Extend Root Device', subject='Instance', detail=None)
def server_hot_extend_root_device(request, instance_id, size, granularity):
    return novaclient(request).servers.hot_extend_root_device(instance_id, size, granularity)


@base.create_log_decorator(optype='Resize', subject='Instance', detail=None)
...
@base.create_log_decorator(optype='Revert Resize', subject='Instance', detail=None)
def server_revert_resize(request, instance_id):
    novaclient(request).servers.revert_resize(instance_id)


@base.create_log_decorator(optype='Start', subject='Instance', detail=None)
def server_start(request, instance_id):
    novaclient(request).servers.start(instance_id)


@base.create_log_decorator(optype='Stop', subject='Instance', detail=None)
def server_stop(request, instance_id):
    novaclient(request).servers.stop(instance_id)


@base.create_log_decorator(optype='Lock', subject='Instance', detail=None)
def server_lock(request, instance_id):
    novaclient(request).servers.lock(instance_id)


@base.create_log_decorator(optype='Unlock', subject='Instance', detail=None)
...
def hypervisor_list(request, server_id = None):
    #return novaclient(request).hypervisors.list(server_id)
    return novaclient(request).hypervisors.list(server_id = server_id)
    ...
    for hyper in hypervisors:
        for server in getattr(hyper, "servers", []):
            try:
                ...
                err_code = err.code
                msg = _("Name: %(name)s ID: %(uuid)s")
                msg = msg % {'name': instance.name, 'uuid': server['uuid']}
                response.append(msg)

    if err_code:
        msg = _('Failed to migrate instances: %s') % ', '.join(response)
        ## raise nova_exceptions.ClientException(err_code, msg)
        result={'state':'failed', 'info': msg}
    else:
...
def tenant_absolute_limits(request, reserved=False, tenant_id=None):
    limits = novaclient(request).limits.get(reserved=reserved, tenant_id=tenant_id).absolute
    limits_dict = {}
    for limit in limits:
        if limit.value < 0:
...
###added by laixf,03-21 for LOCK_UNLOCK####
def service_state(request, host=None,binary=None):
...
@base.create_log_decorator(optype='Create', subject='Aggregates', detail=None)
...
def host_ksm_ctl(request, host_name, action):
    func = "%s_ksm" % action
    method = getattr(novaclient(request).hosts, func, None)
    if method is None:
        return {"result": "err: no such action '%s' for ksm" % action}
    return method(host_name)


def host_get_zram_status(request, host_name):
    return novaclient(request).hosts.get_zram_status(host_name)


def host_zram_ctl(request, host_name, action):
    func = "%s_zram" % action
    method = getattr(novaclient(request).hosts, func, None)
    if method is None:
        return {"result": "err: no such action '%s' for zram" % action}
    return method(host_name)
...
def server_group_get(request, sg_id):
    return novaclient(request).server_groups.get(sg_id)
...
def server_group_delete(request, sg_id):
    return novaclient(request).server_groups.delete(sg_id)
