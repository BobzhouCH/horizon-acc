...
def list_loadbalancers_for_all(request, **kwargs):
    loadbalancers = neutronclient(request)\
        .list_loadbalancers(retrieve_all=True, **kwargs).get('loadbalancers')
    return loadbalancers

def list_loadbalancers_for_pool(request):
...
def _pool_get(request, pool_id, expand_resource=False):
    ...
    if expand_resource:
        ...
        try:
            ...
            messages.warning(request, _("Unable to get subnet for pool %(pool)s.") % {"pool": pool_id})
        ...
        try:
            ...
            messages.warning(request, _("Unable to get members for pool %(pool)s.") % {"pool": pool_id})
        try:
            pool['health_monitors'] = pool_health_monitor_list(
                request, id=pool['health_monitors'])
        except Exception:
            messages.warning(request,
                             _("Unable to get health monitors for pool %(pool)s.") % {"pool": pool_id})
    return Pool(pool)
...
def pool_list_for_quota(request):
