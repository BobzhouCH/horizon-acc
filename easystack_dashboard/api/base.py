...
def create_log_decorator(optype, subject, detail):

    def _log_success_decorator(request, result, create_time, *args, **kwargs):
        LOG.debug(('{"Action_Type": "%(event_type)s", '
                    '"Time": "%(time)s", '
                    '"Object": "%(event_object)s", '
                    '"Subject": "%(subject)s", '
                    '"User_Address": "%(user_address)s", '
                    '"Result": "%(result)s", '
                    '"Level": "%(level)s", '
                    '"Detail": "%(detail)s"}') %
                  dict(event_type=_(optype),
                       time=create_time,
                       event_object=request.user.username,
                       subject=_(subject),
                       user_address=_get_remote_ip(request),
                       result=_('Success'),
                       level=_(str(detail['level']) if type(detail)==dict and detail.has_key('level') else 'medium_level'),
                       detail=detail))

    def _log_error_decorator(request, result, create_time, *args, **kwargs):
        LOG.debug(('{"Action_Type": "%(event_type)s", '
                  '"Time": "%(time)s", '
                  '"Object": "%(event_object)s", '
                  '"Subject": "%(subject)s", '
    ...
    def wrapper(func):
        def inner(request, *args, **kwargs):
            ...
            try:
                ...
                _log_success_decorator(request, result, create_time,
                                       *args, **kwargs)
            except Exception as e:
                LOG.error(e)
                # TODO(need to fix):
                _log_error_decorator(request, lambda result: result or {}, create_time,
                                       *args, **kwargs)
                # raise exception to Servers post() in rest api.
                raise e
            return result
        return inner
    return wrapper
