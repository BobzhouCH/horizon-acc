...
@base.create_log_decorator(optype='Create', subject='Firewall Rules', detail=None)
...
def _rule_list(request, expand_policy, **kwargs):
    ...
    if expand_policy and rules:
        ...
        for rule in rules:
            ...
            rule['policy'] = rule['policy'].to_dict() \
                if rule['policy'] else None
...
@base.create_log_decorator(
    optype='Create', subject='Firewall Policies', detail=None)
def policy_create(request, expand_rule=True, **kwargs):
    ...
    if expand_rule and policy:
        ...
        policy['rules'] = [rule_dict.get(rule).to_dict()
...
def _policy_list(request, expand_rule, **kwargs):
    ...
    if expand_rule and policies:
        ...
        rule_dict = SortedDict((rule.id, rule) for rule in rules)
        for p in policies:
            p['rules'] = [rule_dict.get(rule).to_dict()
                          for rule in p['firewall_rules']]
...
@base.create_log_decorator(optype='Create', subject='Firewall', detail=None)
...
def _firewall_list(request, expand_policy, **kwargs):
    ...
    if expand_policy and firewalls:
        ...
        policy_dict = SortedDict((p.id, p) for p in policies)
        for fw in firewalls:
            fw['policy'] = policy_dict.get(fw['firewall_policy_id']).to_dict()
