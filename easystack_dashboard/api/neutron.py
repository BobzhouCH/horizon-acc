...
class QosPolicy(NeutronAPIDictWrapper):
    """Wrapper for qos policy """

    def __init__(self, apiresource):
        super(QosPolicy, self).__init__(apiresource)

class BandWidthLimitRule(NeutronAPIDictWrapper):
    """Wrapper for bind width limit ruler"""

    def __init__(self, apiresource):
        super(BandWidthLimitRule, self).__init__(apiresource)
...
class SecurityGroup(NeutronAPIDictWrapper):
    ...
    def to_dict(self):
        return dict((k, self._apidict[k]) for k in self._apidict if k != 'rules')
...
class SecurityGroupRule(NeutronAPIDictWrapper):
    ...
    def __unicode__(self):
        ...
        return (_('ALLOW %(ethertype)s %(proto_port)s %(direction)s %(remote)s') %
...
class FloatingIpManager(network_base.FloatingIpManager):
    ...
    def list_pools(self, **filter):
        ...
        return [FloatingIpPool(pool) for pool
                in self.client.list_networks(**filter).get('networks')]

    def pool_get(self, pool):
        return FloatingIpPool(self.client.show_network(pool).get('network'))
    ...
    def _set_instance_info(self, fip, port=None):
        # router has no fixed_ip_address
        if fip['fixed_ip_address'] is None and fip['router_id']:
            fip['instance_id'] = fip['router_id']
            fip['instance_type'] = 'router'
        elif fip['port_id'] is not None:
            port_id = fip['port_id']
            if port_id and not port:
                try:
                    port = port_get(self.request, port_id)
                except exceptions.NotFound as e:
                    LOG.debug('failed to port_get(%s): %s', port_id, e)
            if port:
    ...
    def update_bandwith(self, floating_ip_id, bandwidth):
        update_dict = {'bandwidth': bandwidth}
        self.client.update_floatingip(floating_ip_id,
                                      {'floatingip': update_dict})

    def allocate(self, pool, bandwidth):
        if bandwidth:
...
def network_ip_availabilities_list(request, **params):
    """Fetches IP availibility information for all networks"""
    LOG.debug("network_ip_availabilities_list(): params = %s" % params)
    network_ip_availabilities = neutronclient(request).list_network_ip_availabilities(**params)
...
@base.create_log_decorator(optype='Create', subject='Network', detail=None)
...
def subnet_get(request, subnet_id, **params):
    ...
    subnet['network'] = {}
    if subnet:
        network = neutronclient(request).show_network(subnet['network_id'],
                                                      **params).get('network')
...
def security_group_get(request, sg_id):
    sg = neutronclient(request).show_security_group(sg_id).get('security_group')
    return sg



@base.create_log_decorator(optype='Create', subject='Subnet', detail=None)
...
def subnet_update(request, subnet_id, **kwargs):
    ...
    body = {'subnet': kwargs}
    old = subnet_get(request, subnet_id, **kwargs)
    if body['subnet']['gateway_ip'] and body['subnet']['gateway_ip'] == old['_apidict']['gateway_ip']:
      del body['subnet']['gateway_ip']
...
@base.create_log_decorator(
    optype='Create', subject='Virtual Interfaces', detail=None)
...
@base.create_log_decorator(optype='Create', subject='Router', detail=None)
@billing.create_product('router', uuid_name='id')
...
def router_add_gateway(request, router_id, network_id, ip_addresses=None):
    if ip_addresses:
        body = {'network_id': network_id, "external_fixed_ips": ip_addresses}
    else:
...
def router_port_list(request, router_id):
    return port_list(request, device_id=router_id)
...
def get_feature_permission(request, feature, operation=None):
    ...
    if feature_policies and policy_check:
        ...
        if not policy_name:
            # Translators: Only used inside Horizon code and invisible to users
            raise ValueError(_("The 'operation' parameter for get_feature_permission '%(feature)s' is invalid. It should be one of %(allowed)s")
                             % {'feature': feature,
...
def qos_policy_list(request, **params):
    LOG.debug("qos_policy_list(): params = %s" % params)
    qos_policies = neutronclient(request).list_qos_policies(**params).get('policies')
    return [QosPolicy(n) for n in qos_policies]


def create_qos_policy(request, **kwargs):
    LOG.debug("create_qos_policy(): kwargs = %s" % kwargs)
    if 'tenant_id' not in kwargs:
        kwargs['tenant_id'] = request.user.project_id
    body = {'policy': kwargs}
...
def qos_policy_delete(request, policy_id):
    neutronclient(request).delete_qos_policy(policy_id)


def qos_policy_update(request, qos_policy_id, **kwargs):
    LOG.debug("qos_policy_update(): params = %s" % kwargs)
    body = {'policy': kwargs}
    policy = neutronclient(request).update_qos_policy(qos_policy_id, body=body).get('policy')
    return QosPolicy(policy)


def get_qos_policy(request, qos_policy_id, **kwargs):
    LOG.debug("get_qos_policy(): qos_policy_id = %s" % qos_policy_id)
    policy = neutronclient(request).show_qos_policy(qos_policy_id, **kwargs).get('policy')
    return QosPolicy(policy)


def bandwidth_limit_rule_create(request, policy_id, **kwargs):
    LOG.debug("bandwidth_limit_rule_create(): policy_id = %s" % policy_id)
    body = {'bandwidth_limit_rule': kwargs}
    bandwidth_limit_rule = neutronclient(request).create_bandwidth_limit_rule(policy_id
                                                                              , body=body)
    return BandWidthLimitRule(bandwidth_limit_rule)


def bandwidth_limit_rule_update(request, rule_id,
                                policy_id, **kwargs):
    LOG.debug("bandwidth_limit_rule_update(): rule_id = %s" % rule_id)
    body = {'bandwidth_limit_rule': kwargs}
    bandwidth_limit_rule = neutronclient(request).update_bandwidth_limit_rule(rule_id,
                                                                              policy_id, body=body)
    return BandWidthLimitRule(bandwidth_limit_rule)


def bandwidth_limit_rule_delete(request, rule_id, policy_id):
    LOG.debug("bandwidth_limit_rule_delete(): rule_id = %s" % rule_id)
    neutronclient(request).delete_bandwidth_limit_rule(rule_id, policy_id)


def bandwidth_limit_rules_list(request, policy_id, **kwargs):
    LOG.debug("bandwidth_limit_rules_list(): policy_id = %s" % policy_id)
    bandwidth_limit_rules = neutronclient(request).\
        list_bandwidth_limit_rules(policy_id, **kwargs).get('bandwidth_limit_rules')
    return [BandWidthLimitRule(n) for n in bandwidth_limit_rules]


def bandwidth_limit_rule_show(request, rule_id, policy_id):
    LOG.debug("bandwidth_limit_rule_delete(): rule_id = %s" % rule_id)
    neutronclient(request).show_bandwidth_limit_rule(rule_id, policy_id, body=None)
