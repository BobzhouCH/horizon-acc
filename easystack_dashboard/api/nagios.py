...
class Nagios:
    '''This class represents the current state of
     a Nagios installation, as read
    from the status file that Nagios maintains.

    '''
    def __init__(self, statusfile):
        '''Create a new Nagios state store.  One argument,
         statusfile, is used to
        indicate where the status file is.
          This object is intended to be read-only
    ...
    def _update(self, statusfile):
        '''Read the status file from Nagios and parse it.
            ...
            for line in f:
                ...
                if line.endswith('{'):
                    ...
                    if line.find("NAGIOS STATE RETENTION FILE") is not -1:
                        raise ValueError(
                            "You appear to have used the state retention"
                            "file instead of the status file. Please change"
        ...
        for obj in next_stanza(f):
            ...
            service = obj['service_description'] if \
                'service_description' in obj else None
...
class HostOrService(NagiosObject):
    ...
    def __init__(self, obj):
        ...
        self.comments = {}
        self.essential_keys = [
            'current_state', 'plugin_output',
            'notifications_enabled', 'last_check',
            'last_notification',
            'active_checks_enabled', 'problem_has_been_acknowledged',
            'last_hard_state', 'scheduled_downtime_depth',
            'performance_data', 'last_state_change',
...
class Comment(NagiosObject):
    ...
    def __init__(self, obj):
        ...
        NagiosObject.__init__(self, obj)
        self.essential_keys = [
            'comment_id', 'entry_type', 'source',
            'persistent', 'entry_time', 'expires',
            'expire_time', 'author',
            'comment_data']
...
class Downtime(NagiosObject):
    ...
    def __init__(self, obj):
        ...
        NagiosObject.__init__(self, obj)
        self.essential_keys = [
            'downtime_id', 'entry_time', 'start_time',
            'end_time', 'triggered_by', 'fixed',
            'duration', 'author',
            'comment']
        self.downtime_id = int(self.downtime_id)


def nagios_detailed(request, filters=None, paginate=False):
    '''Monitor the Nagios status
    file and update our global data store
    with the data as it changes.

    '''
    nagios_file = getattr(settings, 'NAGIOS_URL',
                          '/usr/local/nagios/var/status.dat')
    services = []
    if not os.path.exists(nagios_file):
        return services
    NAGIOS = Nagios(nagios_file)
    if NAGIOS.services.get(filters):
        services = NAGIOS.services.get(filters).values()
    else:
        LOG.error("Does not fond the Nagios host %s" % filters)
    return services
