...
class AdminVolumeType(BaseCinderAPIResourceWrapper):

    _attrs = ['id', 'name', 'extra_specs', 'created_at', 'encryption',
              'associated_qos_spec_id', 'associated_qos_spec', 'description', 'is_public',
              'os-extended-snapshot-attributes:project_id']
...
class VolumeBackup(BaseCinderAPIResourceWrapper):
    ...
    _attrs = ['id', 'name', 'description', 'container', 'size', 'status', 'has_dependent_backups',
              'fail_reason', 'created_at', 'volume_id', 'volume_name', 'availability_zone', 'is_incremental']
    _volume = None
...
def volume_list(request, search_opts=None):
    ...
    transfer_volume_list = transfer_list(request, search_opts=search_opts)
    transfers = {}
    for v in transfer_volume_list:
        transfers.update({v.volume_id: v})
...
@base.create_log_decorator(optype='Create', subject='Volume', detail=None)
@billing.create_product('volume', uuid_name='id', quantity='size')
def volume_create(request, size, name, description, volume_type,
                  snapshot_id=None, multiattach=False, metadata=None, image_id=None,
...
@base.create_log_decorator(
    optype='Create', subject='Volume Snapshot', detail=None)
@billing.create_product('snapshot', uuid_name='id')
...
def volume_backup_list(request,search_opts=None):
    ...
    if c_client is None:
        return []
    return [VolumeBackup(b) for b in c_client.backups.list(detailed=True, search_opts=search_opts)]


@base.create_log_decorator(
    optype='Create', subject='Volume Backup', detail=None)
@billing.create_product(ptype='backup-full', uuid_name='id', quantity='size')
...
@billing.create_product(ptype='backup-increment', uuid_name='id', quantity='size')
def volume_backup_incre_create(request,
                         volume_id,
                         name,
                         incremental,
                         description):
...
def volume_backup_restore_default(request, backup_id):
    return cinderclient(request).restores.restore(backup_id=backup_id)
...
def volume_type_list_with_qos_associations(request):
    ...
    for qos_spec in qos_specs:
        ...
        for assoc_vol_type in assoc_vol_types:
            ...
            vol_type.associated_qos_spec_id = qos_spec.id
...
@base.create_log_decorator(
    optype='Create', subject='Volume Type', detail=None)
...
@base.create_log_decorator(
    optype='Create', subject='Volume Encryption Type', detail=None)
...
@base.create_log_decorator(
    optype='Create', subject='Qos Specs', detail=None)
