...
class BaseUsage(object):
        ...
        if not hasattr(self, "start") or not hasattr(self, "end"):
            ...
            if form.is_valid():
                ...
                messages.error(self.request,
                               _("Invalid date format: Using today as default."))
    ...
    def summarize(self, start, end):
        ...
        if start <= end and start <= self.today:
            ...
            messages.error(self.request,
                           _("Invalid time period. The end date should be more recent than the start date."))
        elif start > self.today:
            messages.error(self.request,
                           _("Invalid time period. You are requesting data from the future which may not exist."))
