...
(function () {
  ...
  angular
    ...
    .controller('LaunchBareInstanceFlavorController', LaunchInstanceFlavorController);
  ...
  function LaunchInstanceFlavorController($scope, quotaChartDefaults, launchInstanceModel) {
    ...
    ctrl.lenovoFlavorStepConfig = launchInstanceModel.lenovoFlavorStepConfig || [];
    ctrl.lenovoFlavorVolumeConfig  = launchInstanceModel.lenovoFlavorVolumeConfig || [];
    ...
    //$scope.searchFlavor = searchFlavor;
    $scope.searchFlavor=searchFlavor;
    $scope.availableFlavorFacadesCopy =ctrl.availableFlavorFacades;
    ...
    function getErrors(flavor) {
      ...
      if (source && sourceType && (sourceType.type === 'image' || sourceType.type === 'snapshot')) {
    ...
    function searchFlavor(input){
      ctrl.availableFlavorFacades = $scope.availableFlavorFacadesCopy;
      var displayedAvailableFlavorFacadesFiltered =[];
      angular.forEach(ctrl.availableFlavorFacades,function(item){
        if(item.name.indexOf(input)>-1){
          displayedAvailableFlavorFacadesFiltered.push(item);
        }
      });
      ctrl.availableFlavorFacades=displayedAvailableFlavorFacadesFiltered;
    }
  }
})();
