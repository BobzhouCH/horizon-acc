...
horizon.network_topology = {
  ...
  init:function() {
    ...
    self.balloon_firewallTmpl = Hogan.compile(angular.element('#balloon_firewall').html());
  ...
  new_node: function(data, x, y) {
    ...
    var node = self.vis.selectAll('.node').data(self.nodes);
    var nodeEnter = node.enter().append('g')
      .attr({ 'class': 'node', 'ng-click': 'ab()' })
      .style('fill', 'white')
      ...
      case 'path':
        nodeEnter.append('path')
          ...
          .attr('transform', function(d) {
            switch (Object.getPrototypeOf(d.data)) {
              case Firewall.prototype:
                return 'scale(0.13)translate(-160,-160)'
              default:
  ...
  load_topology: function(data) {
    ...
    var net, _i, _netlen, _netref, rou, _j, _roulen, _rouref, port, _l, _portlen, _portref,
        ser, _k, _serlen, _serref, fire, _m, _firelen, _fireref, obj, vmCount;
    var change = false;
    var filterNode = function(obj) {
      return function(d) {
        return obj == d.data;
    ...
    // Firewalls
    _fireref = data.firewalls;
    for (_m = 0, _firelen = _fireref.length; _m < _firelen; _m++) {
      fire = _fireref[_m];
      var firewall = new Firewall(fire);
      if (!self.already_in_graph(self.data.firewalls, firewall)) {
        self.new_node(firewall);
        change = true;
      } else {
        obj = self.find_by_id(firewall.id);
        if (obj) {
          firewall.routers = obj.routers;
          obj.data = firewall;
        }
      }
      self.data.firewalls[firewall.id] = firewall;
      if (firewall.routers) {
        var firewall_tmp = self.find_by_id(firewall.id);
        for (var router_counter = 0; router_counter <= firewall.routers.length-1; router_counter++) {
          var router_tmp = firewall.routers[router_counter];
          if (firewall_tmp && self.find_by_id(router_tmp)) {
            self.new_link(self.find_by_id(router_tmp), firewall_tmp);
          }
        }
      }
    }
        ...
        if (angular.isDefined(device) && angular.isDefined(_network)) {
          if (port.device_owner == 'compute:nova' || port.device_owner == 'compute:None') {
            ...
            if (port.fixed_ips) {
              ...
              for(var i=0; i<port.fixed_ips.length; i++) {
                device.data.ip_addresses.push(port.fixed_ips[i]);
              }
            }
          ...
          if (_network.data.collapsed) {
            ...
            if (port.fixed_ips) {
              ...
              for(var k=0; k<port.fixed_ips.length; k++) {
                server.ip_addresses.push(port.fixed_ips[k]);
              }
            }
            self.new_node(server);
  ...
  show_balloon: function(d,d2,element) {
    ...
    var firewallTmpl = self.balloon_firewallTmpl;
    ...
    if (d.hasOwnProperty('ports')) {
      angular.element.each(d.ports, function(i, port) {
        ...
        object.url = port.url;
        object.port_status = gettext(port.status);
        ...
        try {
          ...
          for(var i=0; i<port.fixed_ips.length; i++){
            ipAddress += port.fixed_ips[i].ip_address + ' ';
          }
        }catch(e) {
        ...
        object.device_owner = gettext(deviceOwner);
    ...
    if (d instanceof Router) {
      ...
      htmlData.delete_label = gettext('Delete Firewall');
      htmlData.view_details_label = gettext('View Firewall Details');
      htmlData.console_id = d.id;
      htmlData.rules = d.rules.map(function(rule) {
        return {'id': rule};
      });
      html = balloonTmpl.render(htmlData,{
        table1:deviceTmpl,
  ...
  svgs: function(name) {
    switch (name) {
      ...
      case 'firewall':
        return 'm 250.149,248.839 -109.31,0 c -2.348,0 -4.252,-1.881 -4.252,-4.202 l 0,-26.415 c 0,-2.32 1.903,-4.202 4.252,-4.202 l 109.31,0 c 2.349,0 4.252,1.882 4.252,4.202 l 0,26.415 c -0.001,2.321 -1.903,4.202 -4.252,4.202 z m -0.303,-77.743 c 2.348,0 4.252,1.883 4.252,4.203 l 0,26.413 c 0,2.324 -1.904,4.203 -4.252,4.203 l -51.012,0 c -2.347,0 -4.252,-1.879 -4.252,-4.203 l 0,-26.413 c 0,-2.32 1.904,-4.203 4.252,-4.203 l 51.012,0 z m 0.303,-7.503 -109.31,0 c -2.348,0 -4.252,-1.882 -4.252,-4.202 l 0,-26.416 c 0,-2.32 1.903,-4.201 4.252,-4.201 l 109.31,0 c 2.349,0 4.252,1.88 4.252,4.201 l 0,26.416 c -0.001,2.32 -1.903,4.202 -4.252,4.202 z m -70.749,-42.626 0,-4.201 c -0.644,-13.82 -5.903,-24.941 -7.287,-20.712 -1.384,4.229 -4.554,10.506 -4.554,10.506 -8.456,12.011 -6.984,-28.518 -6.984,-28.518 -21.122,18.298 -17.222,40.407 -16.744,42.722 l -15.441,-0.097 c 0,0 -25.078,-40.698 39.473,-78.644 0,0 -2.211,15.535 17.005,28.817 0.679,0.519 3.582,3.082 4.858,4.803 12.342,16.675 8.859,41.872 5.769,45.324 l -16.095,0 0,0 z m -51.619,12.008 0,26.416 c 0,2.32 -1.903,4.202 -4.251,4.202 l -51.012,0 c -2.347,0 -4.251,-1.882 -4.251,-4.202 l 0,-26.416 c 0,-2.32 1.903,-4.201 4.251,-4.201 l 51.013,0 c 2.348,-0.001 4.25,1.88 4.25,4.201 z m -55.263,38.121 109.312,0 c 2.348,0 4.252,1.883 4.252,4.203 l 0,26.413 c 0,2.324 -1.904,4.203 -4.252,4.203 l -109.312,0 c -2.347,0 -4.251,-1.879 -4.251,-4.203 l 0,-26.413 c 0.001,-2.32 1.904,-4.203 4.251,-4.203 z m 0,42.925 51.013,0 c 2.348,0 4.251,1.882 4.251,4.202 l 0,26.415 c 0,2.321 -1.903,4.202 -4.251,4.202 l -51.013,0 c -2.347,0 -4.251,-1.881 -4.251,-4.202 l 0,-26.415 c 0.001,-2.321 1.904,-4.202 4.251,-4.202 z'
      default:
        return '';
    }
  },
