...
(function () {
  ...
  function settingsService($q, apiService) {
    ...
    service.getSetting = function (path, defaultSetting) {
      ...
      function onError() {
        var message = gettext('Unable to retrieve setting.');
        if (!suppressError && horizon.alert) {
          horizon.alert('error', message);
        }
        return message;
      ...
      return apiService.get('/api/setting/' + pathElements[0])
        .error(onError)
        .then(
          function(response){
            ...
            return resur(response.data, pathElements);
          },
          function(response){
            return defaultSetting;
          });
