...
@register.inclusion_tag('easystack_dashboard/_es_accordion_nav.html', takes_context=True)
def horizon_nav(context):
    ...
    # for mana
    cloud_admin = api.keystone.is_cloud_admin(context.get('request'))
    in_mana = api.keystone.is_dedicated_context(context.get('request'))
    dedicated_resource = context['request'].session.get('dedicated_resource')
    dedicated_id = context['request'].session.get('dedicated_id')
    if dedicated_id is None:
        for project in context['authorized_tenants']:
            if project.name == 'dedicated:default':
                dedicated_id = project.id
                context['request'].session['dedicated_id'] = project.id
                break
    ...
    for dash in Horizon.get_dashboards():
        ...
        if (callable(dash.nav) and dash.nav(context) and
            ...
            if dash.slug == 'mana':
                mana_dashboards.append((dash, SortedDict(non_empty_groups)))
            else:
            ...
            if dash.slug == 'mana':
                ...
                dashboards.append((dash, SortedDict(non_empty_groups)))
    return {'mana_components': mana_dashboards,
            'mana_enable': mana_enable,
...
@register.inclusion_tag('easystack_dashboard/_es_subnav_list.html', takes_context=True)
